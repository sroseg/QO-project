...
#time parameters
Tfinal = 1111 #duration of the adiabatic evolution
N_steps = 100000
dt = Tfinal/N_steps
...
# Prepare the Final Hamiltonian in vector form
...
Ham = Hfdiag - H0
U_A = linalg.expm(-1j*H0*dt/(2))
U_B = linalg.expm(-1j*Ham*dt**2/(2*Tfinal))
U_BPlus = linalg.expm(-1j*Ham*dt**2/(2*Tfinal))
              

t = 0
Psi = PsiInit
while t < Tfinal:    
#    U_B = linalg.expm(-1j*(dt**2)/(2*Tfinal)*Ham)
#    U_B = linalg.expm(-1j*S_func(t+dt/2)*Ham*dt)
    Psi = U_A @ Psi # matrix product of first term (U_A) and Psi
    Psi = U_B @ Psi # matrix product of second term (U_B) and Psi
    Psi = U_A @ Psi # matrix product of third term (U_A) and Psi
    U_B = U_B @ U_BPlus 
    t += dt
